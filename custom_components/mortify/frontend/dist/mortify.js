...
class MortifyPanel extends HTMLElement {
  ...
  async _createAndStart() {
    ...
    if (s.selectedEntities.size < 4) {
      showToast(this._root, `Pick at least 4 entities (have ${s.selectedEntities.size})`);
      return;
    }
    s.generating = true;
    this._render();
  ...
  _renderSetup() {
    ...
    // Section 4: Entities
    wrap.appendChild(this._sectionCard(4, "Clue Entities",
      "Pick at least 4 devices — motion sensors, lights, locks, switches. " +
      "These become physical clues in the home during the mystery.",
      this._renderEntityPicker(),
    ));

    // Section 5: Game settings
    wrap.appendChild(this._sectionCard(5, "Game Settings", "",
    ...
    // Begin button
    const ready = s.selectedAgent && s.selectedEntities.size >= 4;
    const launchRow = el("div", { class: "mty-section-launch" }, [
      ...
      el("div", { class: "mty-hint" },
        ...
        s.selectedEntities.size < 4 ? `${s.selectedEntities.size}/4 entities picked` :
        `Ready — ${s.selectedEntities.size} entities, ${s.suspectCount} suspects`),
    ]);
  ...
  _renderQrCard(joinCode) {
    ...
    return el("div", { class: "mty-card" }, [
      ...
      el("div", { class: "mty-qr-box" }, [
        el("img", {
          class: "mty-qr-code",
          src: `/api/mortify/qr?data=${encodeURIComponent(joinUrl)}`,
          alt: "QR code",
        }),
      ]),
...
class PlayerClient {
  ...
  _onMessage(ev) {
    ...
    if (event === "clue_result") {
      // Append to the player's view of clues. Don't track here — game state
      // already updates the count; this is just for the toast / narration.
      showToast(this.root, `Clue: ${data.clue_text || "(unknown)"}`);
  ...
  doDiscover(entityId) {
    this.send({ type: "discover_clue", entity_id: entityId });
  }
  ...
  _renderGame() {
    ...
    if (tab === "suspects") {
      ...
      const myClues = new Set(you.clues_found || []);
      if (!entities.length) {
        wrap.appendChild(el("div", { class: "mty-empty" }, "No entities in this game."));
      } else {
        wrap.appendChild(el("div", { class: "mty-clue-list" },
          entities.map(e => el("div", {
            class: "mty-clue-row" + (myClues.has(e.entity_id) ? " mty-clue-row-found" : ""),
            onClick: () => {
              if (myClues.has(e.entity_id)) return;
              this.doDiscover(e.entity_id);
            },
          }, [
            el("div", { class: "mty-clue-row-icon" }, myClues.has(e.entity_id) ? "✓" : "🔍"),
            el("div", {}, [
              el("div", { class: "mty-clue-row-name" }, e.name),
              el("div", { class: "mty-clue-row-area" }, `${e.area || "Unknown Room"} · ${e.state}`),
            ]),
          ])),
        ));
      }
  ...
  _renderChat() {
    ...
    const wrap = el("div", { class: "mty-chat-screen" });
    wrap.appendChild(el("div", { class: "mty-chat-suspect" }, [
      el("button", {
        class: "mty-chat-back",
        onClick: () => { this.state.activeSuspectId = null; this.render(); },
      }, "← Back"),
      el("div", { class: "mty-suspect-emoji" }, suspect.role_emoji || "🎭"),
      el("div", {}, [
        el("div", { class: "mty-suspect-row-name" }, suspect.role_name || sid),
        el("div", { class: "mty-suspect-row-player" }, suspect.player ? `played by ${suspect.player}` : "NPC"),
      ]),
    ]));

    const log = el("div", { class: "mty-chat-log" });
    if (!messages.length) {
      log.appendChild(el("div", { class: "mty-empty" }, "Ask them anything. Press them. Watch their eyes."));
    } else {
      for (const m of messages) {
        log.appendChild(el("div", {
          class: "mty-msg " + (m.from === "you" ? "mty-msg-you" : "mty-msg-suspect"),
        }, m.text));
      }
    }
    if (this.state.sending) {
      log.appendChild(el("div", { class: "mty-msg mty-msg-suspect mty-pulse" }, "…thinking…"));
    }
    ...
    // Defer focus so iOS keyboards behave.
