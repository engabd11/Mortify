  ...
  _renderGame() {
    ...
    if (tab === "suspects") {
      ...
      const suspectName = (rid) => {
        ...
        return s ? `${s.role_emoji || "🎭"} ${s.role_name || rid}` : rid;
      };
      if (!clues.length) {
        ...
        wrap.appendChild(el("div", { class: "mty-clue-hint" },
          `${myClues.size} of ${clues.length} clues uncovered. Question the suspects to draw them out.`));
        wrap.appendChild(el("div", { class: "mty-clue-list" },
          clues.map(c => {
            const found = myClues.has(c.id);
            return el("div", {
              class: "mty-clue-row" + (found ? " mty-clue-row-found" : ""),
              onClick: () => { if (found) this.doDiscover(c.id); },
            }, [
              el("div", { class: "mty-clue-row-icon" }, found ? "🔓" : "🔒"),
              el("div", {}, [
                el("div", { class: "mty-clue-row-name" }, found ? c.title : "Locked clue"),
                el("div", { class: "mty-clue-row-area" },
                  found
                    ? `Held by ${suspectName(c.held_by)} — tap to re-read`
                    : `${c.relevance || "Unknown"} · ask ${suspectName(c.held_by)}`),
              ]),
            ]);
          }),
        ));
  ...
  _renderChat() {
    ...
    if (!messages.length) {
      ...
      for (const m of messages) {
        ...
        log.appendChild(el("div", { class: "mty-msg " + cls }, m.text));
      }
    }
    ...
    // Confront control — a one-shot, high-stakes move. Only offered if the
    // player holds a clue and hasn't already spent their confrontation.
    const you = this.state.you || {};
    const myClueIds = you.clues_found || [];
    const alreadyConfronted = you.has_confronted;
    if (myClueIds.length && !alreadyConfronted) {
      const clueList = (game.clues || []).filter(c => myClueIds.includes(c.id));
      ...
      const btn = el("button", {
        ...
        onClick: () => {
          ...
          this.doConfront(sid, select.value);
        },
      }, "Confront");
      ...
      wrap.appendChild(el("div", { class: "mty-confront-spent" },
