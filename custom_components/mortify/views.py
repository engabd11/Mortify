...
class MortifyAPIView(HomeAssistantView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        if action == "state":
            ...
            agents = await self.manager.async_get_conversation_agents()
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        if action == "agents":
            # Return all configured HA conversation agents
            agents = await self.manager.async_get_conversation_agents()
            return web.json_response({"agents": agents})

        elif action == "models":
            # Legacy alias — redirect to agents
            agents = await self.manager.async_get_conversation_agents()
            return web.json_response({"agents": agents})

        elif action == "start":
            agent_entity_id = body.get("agent_entity_id", "")
            if not agent_entity_id:
                return web.json_response({"success": False, "error": "No conversation agent selected"}, status=400)
