...
class Player:
    ...
    # Engagement state
    clues_found: list[str] = field(default_factory=list)  # entity_ids
    ...
    def to_private_dict(self) -> dict[str, Any]:
        ...
        Includes their own accusation, the list of clue entity_ids they've
        personally discovered, and their assigned role. NEVER includes
...
class GameSession:
    ...
    def _score_player(
        ...
        * Each clue discovered: ``POINTS_PER_CLUE_DISCOVERED``.
        * Evidence bonus: if the player both accused correctly AND
          discovered the killer's ``room_clue`` entity, add a bonus.
        """
        player.score = 0
        if player.accusation == killer_id:
            player.score += POINTS_CORRECT_ACCUSATION
        player.score += len(player.clues_found) * POINTS_PER_CLUE_DISCOVERED

        # Evidence bonus: did they actually discover the killer's clue?
        # The killer's room_clue is a free-text string, so we can't match
        # it to an entity_id directly. Instead, we award the bonus if the
        # player accused the killer AND found ≥ 2 clues (proving they
        # investigated rather than guessing).
        if (
            player.accusation == killer_id
            and len(player.clues_found) >= 2
            and killer_suspect is not None
        ):
    ...
    async def discover_clue(
        self, player_id: str, entity_id: str,
    ) -> dict[str, Any]:
        """A player examines an entity. Returns a clue payload.

        Each player can only discover each entity once. The clue text
        comes from one of the suspects' room_clue strings — randomised
        per discovery but deterministic enough that two players who
        examine the same entity get a consistent answer.
        """
        async with self._lock:
            # Clue discovery is allowed during any investigation act
            # (1, 2, 3) AND during the accusation phase — players might
            # want to make one last check before committing. ACT_ORDER is
            # [act_1, act_2, act_3, accusation], i.e. exactly the set of
            # states where investigation should still be possible.
            if self.state not in ACT_ORDER:
            ...
            if entity_id in player.clues_found:
                raise InvalidStateError("Clue already discovered")
            if not self.story:
                raise InvalidStateError("No story yet")

            entity = next(
                (e for e in self.entities if e.get("entity_id") == entity_id),
                None,
            )
            if entity is None:
                raise InvalidStateError("Entity not part of this game")

            # Pick a room_clue from the suspects pool. Deterministic per
            # (entity_id, story) so the same entity gives the same flavour
            # text on every discovery.
            suspects = self.story.get("suspects", [])
            pool = [s.get("room_clue", "") for s in suspects if s.get("room_clue")]
            if pool:
                idx = hash(entity_id) % len(pool)
                clue_text = pool[idx]
            else:
                clue_text = (
                    f"The {entity.get('name', entity_id)} shows signs of "
                    "recent tampering."
                )

            player.clues_found.append(entity_id)
            current_state = ""
            state = self._hass.states.get(entity_id) if self._hass else None
            ...
            payload = {
                ...
                "area": entity.get("area", "Unknown Room"),
                "clue_text": clue_text,
                "current_state": current_state,
            }

        # Emit publicly that this player found A clue (not which one).
        # The detailed clue text is delivered to the discovering player
        # via the WS handler's return path.
        await self._emit(
            EVENT_CLUE_DISCOVERED,
            {
                "player_id": player_id,
    ...
    async def submit_interrogation(
        ...
        reply = await generate_npc_reply(
            llm=llm,
            suspect=suspect,
            story=self.story,
            history=history,
        ...
        # Re-acquire the lock to append to history.
        async with self._lock:
            player = self.players.get(player_id)
        ...
        # Emit a public event so the host sees "Alice questioned the Butler"
