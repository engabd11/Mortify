...
from .llm_client import LocalLLMClient, get_conversation_agents
...
class MortifyGameManager:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self.llm = LocalLLMClient(hass, self.config.get(CONF_LLM_MODEL, "conversation.ollama_conversation"))
    ...
    async def async_get_conversation_agents(self) -> list[dict]:
        """Return all available HA conversation agents (Ollama models etc.)."""
        return await get_conversation_agents(self.hass)
    ...
    async def async_start_game(
        ...
        # Switch to the selected conversation agent
        self.llm = LocalLLMClient(self.hass, agent_entity_id)
        _LOGGER.info("Mortify: using conversation agent %s", agent_entity_id)
