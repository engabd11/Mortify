...
class MortifyQRView(HomeAssistantView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        # Optional target pixel size — the client passes the CSS box size so
        # the PNG is rendered at a sensible resolution for the screen it opens
        # on (sharp on a TV, small on a phone) instead of a fixed huge image.
        # We translate it into a per-module box_size, clamped to sane bounds.
        try:
            target_px = int(request.query.get("size", "0"))
        ...
        def _make() -> bytes:
            ...
            qr.add_data(data)
            qr.make(fit=True)
            # If the caller gave a target pixel size, pick a box_size so the
            # rendered image lands near it (modules + 2*border quiet zone).
            if target_px > 0:
                modules = qr.modules_count + 2 * qr.border
                box = max(2, min(20, round(target_px / max(modules, 1))))
                qr.box_size = box
    ...
    async def handle(self, request: web.Request) -> web.StreamResponse:
        ...
        async def forward(event: dict[str, Any]) -> None:
            ...
            if session is None:
                return
            pid = bound["player_id"]
            # Targeted events (e.g. the human-killer prompt, or the asker's
            # "awaiting" notice) carry a "_target_player_id" — only forward
            # them to that player, and strip the routing field on the way out.
            target = event.get("_target_player_id")
            if target is not None and target != pid:
                return
            clean_event = {k: v for k, v in event.items() if k != "_target_player_id"}
            you = (
                session.players[pid].to_private_dict()
                if pid and pid in session.players else None
            )
            # Enrich the private projection with the FULL text of the clues
            # this player has personally unlocked. The public clue list is now
            # opaque (ids only), so this is how a player reads their own finds.
            if you is not None:
                you["unlocked_clue_details"] = session.clue_details_for(pid)
            payload = {
                **clean_event,
        ...
        try:
            async for msg in ws:
                ...
                try:
                    ...
                    elif msg_type == "confront":
                        await _handle_confront(manager, ws, data, bound)
                    elif msg_type == "killer_respond":
                        await _handle_killer_respond(manager, ws, data, bound)
...
async def _handle_join(manager, ws, data, bound, forward):
    ...
    you["unlocked_clue_details"] = session.clue_details_for(player.player_id)
    await _ws_send(ws, {
        "event": "joined",
...
async def _handle_resume(manager, ws, data, bound, forward):
    ...
    bound["unsubscribe"] = session.subscribe(forward)
    you = player.to_private_dict()
    you["unlocked_clue_details"] = session.clue_details_for(verified)
    await _ws_send(ws, {
        "event": "resumed",
        "player_id": player.player_id,
        "player_token": token,
...
async def _handle_discover_clue(manager, ws, data, bound):
    ...
    if session is None:
        ...
        return
    try:
        payload = await session.discover_clue(bound["player_id"], clue_id)
        ...
        return
    # Private response — only the requesting player gets the clue text.
    await _ws_send(ws, {"event": "clue_result", **payload})


async def _handle_confront(manager, ws, data, bound):
    if bound["session_id"] is None:
        await _ws_send(ws, {
            "event": "error", "code": "not_joined",
            "message": "Join first",
    ...
    clue_id = data.get("clue_id")
    if (
        not isinstance(suspect_role_id, str) or not suspect_role_id
        or not isinstance(clue_id, str) or not clue_id
    ):
        await _ws_send(ws, {
            "event": "error", "code": "bad_request",
            "message": "suspect_role_id and clue_id required",
        })
        return
    session = manager.get_session(bound["session_id"])
    if session is None:
        ...
        return
    try:
        result = await session.confront_suspect(
            bound["player_id"], suspect_role_id, clue_id,
        )
    except InvalidStateError as err:
        await _ws_send(ws, {
            "event": "error", "code": "invalid_state",
        ...
        return
    await _ws_send(ws, {"event": "confront_result", **result})


async def _handle_killer_respond(manager, ws, data, bound):
    """The human killer picks one of their pre-generated evasion replies."""
    if bound["session_id"] is None:
        await _ws_send(ws, {
            "event": "error", "code": "not_joined",
            "message": "Join first",
    ...
    if not isinstance(request_id, str) or not request_id or not isinstance(choice_index, int):
        await _ws_send(ws, {
            "event": "error", "code": "bad_request",
            "message": "request_id and choice_index required",
        })
        return
    session = manager.get_session(bound["session_id"])
    if session is None:
        ...
        return
    try:
        result = await session.killer_respond(
            bound["player_id"], request_id, choice_index,
        )
    except InvalidStateError as err:
        await _ws_send(ws, {
            "event": "error", "code": "invalid_state",
        ...
        return
    await _ws_send(ws, {"event": "killer_respond_result", **result})
